...
public class ContributionServiceTests
{
    ...
    public async Task CreateContributionAsync_WithMissingAccount_ReturnsFailure()
    {
        ...
        var service = new ContributionService(Mock.Of<IContributionRepository>(), accountRepo.Object);

        var result = await service.CreateContributionAsync(new CreateContributionCommand(new DateOnly(2026, 8, 9), 25m, ContributionContributorType.Self, 1));
    ...
    public async Task CreateContributionAsync_WithValidCommand_ReturnsSuccessWithAssignedId()
    {
        ...
        var service = new ContributionService(contributionRepo.Object, accountRepo.Object);
    ...
    [Fact]
    public async Task UpdateContributionAsync_WithMissingAccount_ReturnsFailure()
    {
        var accountRepo = new Mock<IAccountRepository>();
        accountRepo.Setup(r => r.GetAccountByIdAsync(1, It.IsAny<CancellationToken>())).ReturnsAsync((Account?)null);
        ...
        var result = await service.UpdateContributionAsync(new UpdateContributionCommand(1, new DateOnly(2026, 8, 9), 25m, ContributionContributorType.Self, 1));
    ...
    public async Task DeleteContributionAsync_DelegatesToRepository()
    {
        ...
        var service = new ContributionService(repo.Object, Mock.Of<IAccountRepository>());
